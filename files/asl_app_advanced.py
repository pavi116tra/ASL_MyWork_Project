...
# ══════════════════════════════════════════════════════════════════════════════
# LEGACY FEATURE EXTRACTION (93-dim)
# ══════════════════════════════════════════════════════════════════════════════
...
CONF_GOOD    = 0.80    # green, allow confirm
CONF_MEDIUM  = 0.60    # yellow, shown but warn
SMOOTH_N     = 7       # frames in voting buffer
VOTE_THRESH  = 5       # need 5/7 to agree
IMG_SIZE     = 128
HAND_MARGIN  = 30
...
def load_model_and_classes():
    ...
    Returns (model, classes, is_fusion, feature_dim).
    """
...
def hand_quality(hand_lm, frame_shape):
    ...
    Checks:
      - All 21 landmarks visible (z reasonable)
    ...
    if out_of_frame:
        return "POOR", "Hand out of frame"
    if z_range < 0.01:
        return "PARTIAL", "Flat — show 3D hand"
    return "GOOD", "Clear"
...
def draw_left_panel(c, live_disp, quality, quality_msg, vote_label, camera_active):
    ...
    put_text(c, f"Hand: {quality}  —  {quality_msg}", 10, CAM_Y2-12,
    ...
    # Prompt
    put_text(c, "Show hand sign  →  press  C  to capture", 10, CAM_Y2-2,
             scale=0.0)  # invisible — just for layout
...
def draw_right_panel(c, snap_disp, snap_label, top3, state):
    ...
    if snap_disp is not None:
        ...
        # Top-3 predictions on right panel
        if top3:
            rect_alpha(c, xo, CAM_Y2-130, xo+CAM_W, CAM_Y2, color=(0,0,0), alpha=0.7)
            rank_cols = [(80,255,80), (255,220,60), (160,160,160)]
            for rank, (lbl, conf) in enumerate(top3):
                pct  = int(conf*100)
                col  = rank_cols[rank]
                tx   = xo + 14
                ty   = CAM_Y2 - 105 + rank * 38
                put_text(c, f"{rank+1}st" if rank==0 else f"{rank+1}nd" if rank==1 else f"{rank+1}rd",
                         tx, ty, scale=0.55, color=(140,140,140))
                put_text(c, f"{lbl}", tx+50, ty, scale=1.1, color=col, thickness=2)
                put_text(c, f"{pct}%", tx+130, ty, scale=0.8, color=col)
                # Bar
                bw = int((pct / 100) * 350)
                cv2.rectangle(c, (tx+185, ty-18), (tx+185+bw, ty-4), col, -1)
    else:
...
def draw_result_bar(c, state, top3, snap_conf):
    ...
    if state==State.CAPTURED and top3:
        ...
        col = (80,255,80) if is_good else (255,220,60) if is_medium else (120,120,255)

        # Big letter
        (lw,lh),_ = cv2.getTextSize(lbl, FONT, 2.4, 3)
        put_text(c, lbl, 60, mid_y+lh//2, scale=2.4, color=(255,255,80), thickness=3)
        put_text(c, "—", 60+lw+18, mid_y+lh//2-8, scale=1.2, color=(150,150,150))
        put_text(c, f"{pct}% accuracy", 60+lw+70, mid_y+lh//2-6,
                 scale=1.0, color=col)

        if is_good:
            hint = "✓ Click CONFIRM to add  |  DELETE to discard"
            hint_col = (180,255,180)
        elif is_medium:
            hint = "⚠ Uncertain — try repositioning hand"
            hint_col = (255,220,100)
        else:
            hint = "✗ Low confidence — CONFIRM disabled  (need >80%)"
            hint_col = (120,120,255)
        centered_text(c, hint, WIN_W-360, mid_y+30, scale=0.55, color=hint_col)
...
def run_inference_on_frame(frame, hand_lm, model, classes, is_fusion, feature_dim=41):
    ...
    Returns list of (label, confidence) sorted by confidence desc, top-3.
    """
    if feature_dim == 93:
...
def main():
    print("\n" + "="*60)
    print("  ASL Advanced App — Temporal Smoothing + Top-3")
    print("="*60)

    print("\nLoading model...")
    model, classes, is_fusion, feature_dim = load_model_and_classes()
    print(f"  Mode: {'FUSION (image+landmarks)' if is_fusion else 'Landmark-only'}")
    ...
    def do_confirm():
        ...
        print(f"[CONFIRM] '{letter}'  Word: {''.join(word_buf)}")
    ...
    while True:
        ...
        if camera_active:
            ...
            if not ret: break
            raw       = cv2.flip(raw, 1)
            last_clean = raw.copy()  # clean frame for snapshots

            rgb  = cv2.cvtColor(raw, cv2.COLOR_BGR2RGB)
            res  = hands_live.process(rgb)

            quality, quality_msg = "POOR", "No hand"
            ...
            if res.multi_hand_landmarks:
                ...
                # Temporal smoothing: live inference for vote
