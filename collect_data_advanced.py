...
Features:
  ...
  - Saves BOTH:
      - Image: data/images/{LETTER}/img_{timestamp}.jpg
      - Landmarks: data/landmarks/{LETTER}/lm_{timestamp}.npy
...
MIN_CONFIDENCE   = 0.8          # minimum MediaPipe detection confidence (quality check)
...
def collect():
    ...
    feat_root = data_root / "landmarks" # Using data/landmarks/ exactly as requested
    ...
    while True:
        ...
        if res.multi_hand_landmarks and res.multi_handedness:
            ...
            if hand_found:
                ...
                if (not paused
                    ...
                    # Save features as lm_{timestamp}.npy exactly as requested
                    feat_path = feat_root / letter / f"lm_{ts}.npy"
        ...
        status = f"{letter}: {count}/{TARGET_PER_CLASS} images" # Requested format
        ...
        if hand_found:
            q_col = (80, 255, 80)
            q_txt = f"Hand Quality: GOOD ({int(confidence*100)}%)"
        else:
            q_col = (100, 100, 255)
            q_txt = f"Hand Quality: REJECTED  (need >= {int(MIN_CONFIDENCE*100)}%)"
        put_text(canvas, q_txt, 660, 325, scale=0.65, color=q_col)

        # Live feature preview (first 15 features as bar chart)
        put_text(canvas, "Live Features Preview (41-dim):", 660, 360, scale=0.55,
                 color=(180,180,180))
        if feat_vec is not None:
            for fi in range(min(15, len(feat_vec))):
                ...
                cv2.rectangle(canvas, (445, bx), (445-bh, bx+32), col, -1)
        ...
        if key == ord("q") or key == ord("Q"):
            ...
            if hand_found and feat_vec is not None:
                ...
                np.save(str(feat_root / letter / f"lm_{ts}.npy"), feat_vec)
                counts[letter] += 1
                last_capture = now
    ...
    print(f"  Data saved to: data/images/  and  data/landmarks/")
    print("="*55)
