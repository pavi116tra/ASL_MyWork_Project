...
Steps:
  1. Load images + pre-extracted landmark .npy files from data/
  2. Data augmentation (15x per sample)
  3. Train dual-input fusion model (landmark + image CNN)
  4. Hard negative mining on A/E/M/N/S/T/G/H confused classes
  5. Fine-tune with class weights on hard classes
  6. Saves best model to model/asl_fusion_model.h5
...
IMG_SIZE   = 128
# Confused classes including A, E, M, N, S, T, G, H as requested
HARD_CLASS = ["A", "E", "M", "N", "S", "T", "G", "H"]
...
def load_dataset(data_root: str, img_size: int = IMG_SIZE):
    """
    Load images and landmark vectors from:
      data_root/images/{LETTER}/*.jpg
      data_root/landmarks/{LETTER}/*.npy

    Returns (images, features, labels_str) all as lists.
    """
    img_root  = Path(data_root) / "images"
    feat_root = Path(data_root) / "landmarks" # Aligned with data/landmarks/

    images, features, labels = [], [], []

    if not img_root.exists() or not feat_root.exists():
        print(f"[ERROR] data/images/ or data/landmarks/ not found at {data_root}.")
    ...
    for letter in letter_dirs:
        ...
        # Match by stem (timestamp) - replacing lm_ prefix
        feat_map = {f.stem.replace("lm_", ""): f for f in feat_files}
